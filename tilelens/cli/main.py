...
def handle_list_hardware(args):
    """Prints a styled table of all hardware accelerators in the registry."""
    if HAS_RICH and console:
        table = Table(
            title="[bold magenta]TileLens Hardware Accelerator Database[/bold magenta]",
            box=box.ROUNDED,
            header_style="bold cyan",
        )
        table.add_column("Device Key / Alias", style="yellow")
        table.add_column("Name & Vendor", style="white")
        table.add_column("Type", style="green")
        table.add_column("Process", justify="right")
        table.add_column("HBM Bandwidth", justify="right", style="cyan")
        table.add_column("SRAM / Core", justify="right", style="magenta")
        table.add_column("Peak BF16 / FP8", justify="right", style="bold green")
        ...
        console.print(table)
        console.print(
            f"[dim]Tip: You can use short aliases like: [yellow]{', '.join(list(ALIAS_MAP.keys())[:8])}[/yellow][/dim]\n"
        )
    else:
        print_msg("\n" + "=" * 95)
        print_msg("TileLens Hardware Accelerator Database")
        print_msg("=" * 95)
        print_msg(f"{'Key / Alias':<22} | {'Name':<24} | {'Type':<8} | {'HBM BW':<12} | {'SRAM/Core':<12} | {'Peak Compute'}")
        print_msg("-" * 95)
        for key, spec in HARDWARE_DATABASE.items():
            bf16_val = spec.compute.dense_tflops.get(Precision.BF16, spec.compute.dense_tflops.get(Precision.INT8, 0.0))
            fp8_val = spec.compute.dense_tflops.get(Precision.FP8, 0.0)
            ...
            if fp8_val > 0:
                compute_str += f" | {fp8_val:,.0f} FP8"
            print_msg(
                f"{key:<22} | {spec.name:<24} | {spec.hardware_type.value:<8} | "
                f"{spec.memory.hbm_bandwidth_gbs:,.0f} GB/s".ljust(12) + f" | "
                f"{spec.memory.sram_per_core_kb:,.0f} KB".ljust(12) + f" | {compute_str}"
            )
        print_msg("=" * 95 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="TileLens: Hardware-Software Co-Design & Roofline Memory Tile Visualizer for GPUs & TPUs",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    subparsers = parser.add_subparsers(dest="command", help="Command to execute")

    # Command: list-hardware
    subparsers.add_parser("list-hardware", help="List all registered GPU, TPU, and ASIC specifications.")

    # Command: roofline
    p_roof = subparsers.add_parser("roofline", help="Calculate standalone theoretical Roofline limit.")
    p_roof.add_argument("--device", "-d", default="h100", help="Target hardware (e.g. h100, tpu_v5e, a100, b200)")
    ...
    p_gemm.add_argument("--device", "-d", default="h100", help="Target hardware alias (e.g. h100, tpu_v5e)")
    ...
    # Command: compare
    p_comp = subparsers.add_parser("compare", help="Compare GEMM performance across multiple GPUs & TPUs.")
    ...
    p_comp.add_argument("--devices", default="h100,tpu_v5e,tpu_v5p,a100", help="Comma-separated device list")
    ...
    p_viz.add_argument("--devices", default="h100,tpu_v5e,tpu_v5p,a100,b200,rtx4090", help="Comma-separated device list")
